   ...
   linkToken_ (consumes<reco::MuonTrackLinksCollection>(theL3LinksLabel))
{
   ...
   hChi2 = fs->make<TH1D>("hChi2","hChi2",1000,0.0,100.0);
}
...
bool HLTMuonL3PreFilter::hltFilter(Event& iEvent, const EventSetup& iSetup, trigger::TriggerFilterObjectWithRefs & filterproduct) const{
	LogDebug("HLTMuonL3PreFilter") << "HLTMuonL3PreFilter::hltFilter" << std::endl;
	std::cout << "HLTMuonL3PreFilter::hltFilter" << std::endl;
...
//	Test to see if we can use L3MTS -> then run the old implementation:
	...
	if (useL3MTS){
		LogDebug("HLTMuonL3PreFilter") << "HLTMuonL3PreFilter::hltFilter is in mode: useL3MTS" << std::endl;
		std::cout << "HLTMuonL3PreFilter::hltFilter is in mode: useL3MTS" << std::endl;
	   ...
	   std::cout <<"looking at: "<<L2toL3s.size()<<" L2->L3s from: "<<mucands->size() << std::endl;
	   for (; L2toL3s_it!=L2toL3s_end; ++L2toL3s_it){
		 ...
		 for (; iTk!=maxItk; iTk++){
		   ...
		   if (min_DxySig_ > 0 && (tk->dxyError() <= 0 || fabs(tk->dxy(beamSpot.position())/tk->dxyError()) < min_DxySig_)) continue;
		   	  ...
		   	  std::cout << "where max_NormalizedChi2_ = " << max_NormalizedChi2_ << " tk->normalizedChi2(): " << tk->normalizedChi2() << std::endl;
		   hChi2->Fill(tk->normalizedChi2());
		   //normalizedChi2 cut
	   ...
	   return accept;
		...
		LogDebug("HLTMuonL3PreFilter") << "HLTMuonL3PreFilter::hltFilter is in mode: not useL3MTS" << std::endl;
		std::cout << "HLTMuonL3PreFilter::hltFilter is in mode: not useL3MTS" << std::endl;
...
//			for(reco::MuonTrackLinksCollection::const_iterator link = links->begin(); link != links->end(); ++link){        //Looping over all links
...
//				std::cout << "link trackerTrack: " << trkTrack.get()  << std::endl;
//				std::cout << "link globalTrack: " << link->globalTrack().get() << std::endl;
//				std::cout << "link standAloneTrack: " << link->standAloneTrack().get() << std::endl;
//				std::cout << "tk: " << tk.get() << std::endl;

//				if (link == tk) std::cout << "ITS THE SAME THING" << std::endl; 
...
//						std::cout << "RecoChargedCandidate is matched to obj in MuonTriackLinks" << std::endl;
						useThisLink=true;
					}
				}
				if (useThisLink){
...
//					if (not link->trackerTrack().isNull()) std::cout << " link tk pt " << link->trackerTrack()->pt() << std::endl;
//					if (not link->standAloneTrack().isNull()) std::cout << " sta pt " << link->standAloneTrack()->pt() << std::endl;
//					if (not link->globalTrack().isNull()) std::cout << " global pt " << link->globalTrack()->pt() << std::endl;
