 ...
 *
 *
 *  \author Hugues Brun based on HLTEgammaGenericFilter from Roberto Covarelli (CERN)
 *
 */

#include "HLTrigger/Muon/interface/HLTMuonGenericFilter.h"
...
//
HLTMuonGenericFilter::HLTMuonGenericFilter(const edm::ParameterSet& iConfig) : HLTFilter(iConfig) {
  ...
  isoToken_ = consumes<reco::RecoChargedCandidateIsolationMap>(isoTag_);
  if(!doIsolated_) nonIsoToken_ = consumes<reco::RecoChargedCandidateIsolationMap>(nonIsoTag_);
}

void
HLTMuonGenericFilter::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
  ...
  desc.add<edm::InputTag>("L1IsoCand",edm::InputTag("hltL1IsoRecoChargedCandidate"));
  desc.add<edm::InputTag>("L1NonIsoCand",edm::InputTag("hltL1NonIsoRecoChargedCandidate"));
  descriptions.add("HLTMuonGenericFilter",desc);
}

HLTMuonGenericFilter::~HLTMuonGenericFilter(){}


// ------------ method called to produce the data  ------------
bool
HLTMuonGenericFilter::hltFilter(edm::Event& iEvent, const edm::EventSetup& iSetup, trigger::TriggerFilterObjectWithRefs & filterproduct) const
{
  ...
  edm::Ref<reco::RecoChargedCandidateCollection> ref;

  // Set output format
  int trigger_type = trigger::TriggerMuon;

  edm::Handle<trigger::TriggerFilterObjectWithRefs> PrevFilterOutput;
  iEvent.getByToken (candToken_,PrevFilterOutput);

  std::vector<edm::Ref<reco::RecoChargedCandidateCollection> > recochargedcands;
  PrevFilterOutput->getObjects(TriggerCluster, recochargedcands);
  if(recochargedcands.empty()) PrevFilterOutput->getObjects(TriggerMuon,recochargedcands);  //we dont know if its type trigger cluster or trigger photon

  //get hold of isolated association map
  edm::Handle<reco::RecoChargedCandidateIsolationMap> depMap;
  iEvent.getByToken (isoToken_,depMap);

  //get hold of non-isolated association map
  edm::Handle<reco::RecoChargedCandidateIsolationMap> depNonIsoMap;
  if(!doIsolated_) iEvent.getByToken (nonIsoToken_,depNonIsoMap);

  // look at all photons, check cuts and add to filter object
  int n = 0;

  for (unsigned int i=0; i<recochargedcands.size(); i++) {

    ref = recochargedcands[i];
    reco::RecoChargedCandidateIsolationMap::const_iterator mapi = (*depMap).find( ref );
